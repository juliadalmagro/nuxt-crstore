<template>
  ...
  <v-container class="justify-center mt-5" style="max-width: 900px;">
    <TabelaDados  titulo="Instrutores" :loading="loading" :headers="headers" :items="items" @edit-item="editItem" @abrir-dialog="() => ativo = true" @delete-item="deleteItem"/>
  </v-container>
  <v-dialog
      v-model="ativo"
      max-width="500"
  ...
  methods: {
    ...
    async persist() {
      try {
        ...
        this.$toast.success(`instrutor ${this.usuario.idUsuario ? 'editado' : 'criado'} com sucesso!`);
      } catch (e) {
        this.$toast.error(`Erro ao ${this.usuario.idUsuario ? 'editar' : 'criar'} instrutor!`);
      }
      this.resetAtividade()
      await this.getItems();
